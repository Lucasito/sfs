 ...
 *
 * @param	{object} [configObj=null]	An object containing the client configuration properties.
 */
...
export class SFSBuddyEvent {
     ...
     * <caption>The object passed to the listener contains the following parameters:</caption>
     * <tr><th>Property</th><th>Type</th><th>Description</th></tr>
     * <tr><td>buddyList</td><td><code>Array of {@link SFSBuddy}</code></td><td>A list of <em>SFSBuddy</em> objects representing all the buddies in the current user's buddy list.</td></tr>
     * <tr><td>myVariables</td><td><code>Array of {@link SFSBuddyVariable}</code></td><td>A list of all the Buddy Variables associated with the current user.</td></tr>
     * </table>
     *
     * @example
...
/**
 * <b>Developers never istantiate the <em>SFSClusterEvent</em> class</b>: only use its static properties.
 *
 * <p>The constants contained in this class are used to register the event listeners; when an event is dispatched, an object containing event-specific parameters is passed to the listener.
 * See the documentation below for a description of the parameters available for each event.</p>
 *
 ...
 */
export class SFSClusterEvent {
    constructor();
    /**
     * The <em>connectionRequired</em> event type, dispatched when the Lobby has found a Game Node for the client to join in a SmartFoxServer 2X Cluster.
     *
     * <p>This event is fired when the Lobby signals that the client should join a given Game Node, in response to a <em>ClusterJoinOrCreateRequest</em> request or when an invitation to join an existing game on a Game Node is accepted by the invited user.</p>
     *
     * <table class="jsdoc-details-table">
     * <caption>The object passed to the listener contains the following parameters:</caption>
     * <tr><th>Property</th><th>Type</th><th>Description</th></tr>
     * <tr><td>configObj</td><td>object</td><td>The pre-populated client configuration object to start a new connection towards the designated Game Node.</td></tr>
     * <tr><td>userName</td><td>string</td><td>The user name to access the Game Node.</td></tr>
     * <tr><td>password</td><td>string</td><td>A temporary and unique password to access the Game Node.</td></tr>
     * </table>
     *
     * @example
     * <caption>This example shows how to attempt a connection to a Game Node when the event is received.</caption>
     * function someMethod()
     * {
     * 	sfsLobby.addEventListener(SFS2X.SFSClusterEvent.CONNECTION_REQUIRED, onGameNodeConnectionRequired, this);
     * }
     ...
     *
     * @constant {string}
     * @memberof SFSClusterEvent
     *
     * @see	ClusterJoinOrCreateRequest
     * @see	ClusterInviteUsersRequest
     */
    static readonly CONNECTION_REQUIRED: string;
    /**
     * The <em>loadBalancerError</em> event type, dispatched when a cluster-related request cannot be satisfied, typically creating or joining a game in a SmartFoxServer 2X Cluster.
     *
     * <p>For example, this event can be fired in response to the <em>ClusterJoinOrCreateRequest</em> request.</p>
     *
     * <p>The event does not provide further details as the Load Balancer simply queries the available servers and if none is found matching the contextual criteria, the cluster request cannot be completed.</p>
     *
     * @example
     * <caption>This example shows how to handle this event type.</caption>
     * function someMethod()
     ...
     * @see	ClusterInviteUsersRequest
     */
    static readonly LOAD_BALANCER_ERROR: string;
}
...
export class RoomProperties {
    ...
    /**
     * The Room is an <em>MMORoom</em> on the server-side.
     * <p>Requires a <em>BoolMatch</em> instance to be used for values comparison.</p>
     *
     * @constant {string}
     * @memberof RoomProperties
     ...
     * @see	BoolMatch
     */
    static readonly IS_TYPE_MMO: string;
    /**
     ...
     * @see	BoolMatch
     */
    static readonly IS_TYPE_DEFAULT: string;
}
...
/**
 * Creates a new <em>ClusterRoomSettings</em> instance.
 *
 * <p>The instance must be passed to the <em>ClusterJoinOrCreateRequest</em> class constructor.</p>
 *
 * <p>On the server-side, a Game Room is represented on the Game Node by the <em>SFSGame</em> Java class, which extends the <em>Room</em> class
 * providing advanced features such as player matching, game invitations, public and private games, quick game joining, etc.
 * On the client side Game Rooms are regular Rooms with their <em>isGame</em> property set to <code>true</code>.</p>
 *
 * @param	{string} name The name of the Game Room to be created.
 */
export class ClusterRoomSettings {
    constructor(name: string);
    /**
     * Indicates whether the game is public or private.
     *
    ...
    minPlayersToStartGame: number;
    /**
     * In private games, defines a list of <em>SFSUser</em> or <em>SFSBuddy</em> objects representing players to be invited to join the game.
     *
     * @type {SFSUser[]|SFSBuddy[]}
     * @default	null
     */
    invitedPlayers: SFSUser[] | SFSBuddy[];
    /**
     * In private games, defines the number of seconds that the users invited to join the game have to reply to the invitation.
     *
     * <p>The suggested range is 30 to 60 seconds.</p>
     *
     * @type {number}
     * @default	30
     */
...
/**
 * Creates a new <em>ClusterTarget</em> instance.
 *
 * <p>The instance must be passed as <em>target</em> parameter to the <em>ClusterInviteUsersRequest</em> class constructor.</p>
 *
 * @param	{string} nodeId		The identifier of the Game Node where the Room is located.
 * @param	{number} roomId 	The identifier of the Room on the Game Node.
 *
 * @see	ClusterInviteUsersRequest
 * @see SmartFox#nodeId
 * @see SFSRoom#id
 */
export class ClusterTarget {
    constructor(nodeId: string, roomId: number);
    /**
...
/**
 * Creates a new <em>ClusterInviteUsersRequest</em> instance.
 * The instance must be passed to the <em>SmartFox.send()</em> method for the request to be executed.
 *
 * <p>In the SmartFoxServer 2X Cluster environment, this request sends a generic invitation to a list of users to join a Room on a Game Node.
 ...
 * <p>If the invitation is accepted, and the invited user is not yet connected to the target Game Node, the <em>connectionRequired</em> event
 * is dispatched to the their client, followed by a <em>roomJoin</em> event after the connection and login process is completed.
 * In case the client is already connected to the target Game Node, the <em>roomJoin</em> event is received immediately.</p>
 *
 * @param	{ClusterTarget} target					A <em>ClusterTarget</em> object containing the identifiers of the Game Node and Room the users are invited to join.
 * @param	{SFSUser[]|SFSBuddy[]} invitedUsers		A list of <em>SFSUser</em> objects, each representing a user to send the invitation to.
 ...
 */
export class ClusterInviteUsersRequest {
    constructor(target: ClusterTarget, invitedUsers: SFSUser[] | SFSBuddy[], secondsForAnswer: number, params?: SFSObject);
}
 ...
 * The instance must be passed to the <em>SmartFox.send()</em> method for the request to be executed.
 *
 * <p>The Lobby Node searches for a public Game Room that meets the criteria expressed by the passed matching expression in the passed Room Groups.
 ...
 * is created on a Game Node selected by the cluster's load balancing system.</p>
 *
 * <p>In any case, if a game to join can be found or is created, the <em>connectionRequired</em> event is dispatched to the requester's client,
 * followed by a <em>roomJoin</em> event after the connection and login process is completed.
 * In case the client is already connected to the target Game Node, the <em>roomJoin</em> event is received immediately.</p>
 *
 * @param	{MatchExpression} matchExpression				A matching expression that the system will use to search a Game Room where to join the current user; if <code>null</code>, a new Game Room will be created.
 * @param	{string[]} groupList							A list of group names to further filter the search; if <code>null</code>, all groups will be searched.
 * @param	{MMORoomSettings|ClusterRoomSettings} settings	If no Rooms are found through the matching expression, a new Room with the passed settings will be created and the user will auto-join it.
 */
...
/**
 * Creates a new <em>QuickJoinOrCreateRoomRequest</em> instance.
 * The instance must be passed to the <em>SmartFox.send()</em> method for the request to be executed.
 *
 * <p>SmartFoxServer searches for a public Room that meets the criteria expressed by the passed matching expression in the passed Room Groups.
 * <p>If no suitable Room can be found, a new Room is created, based on the passed settings. If a Room is created, the <em>roomAdd</em> event is dispatched to all the users who subscribed the Group to which the Room is associated, including the Room creator. In any case, if a Room to join could be found or was created, the <em>roomJoin</em> event is then dispatched.</p>
 * <p>Error conditions (Room creation error, Room join error) should always be checked adding the appropriate listeners.</p>
 *
 * @param	{MatchExpression} matchExpression	A matching expression that the system will use to search a Room where to join the current user.
 * @param	{string[]} groupList				An array of Group names to which the matching expression should be applied.
 * @param	{RoomSettings} settings				An object containing the Room configuration settings.
 * @param	{SFSRoom} [roomToLeave=null]		A <em>SFSRoom</em> object representing the Room that the user should leave when joining the game.
 */
export class QuickJoinOrCreateRoomRequest {
    constructor(matchExpression: MatchExpression, groupList: string[], settings: RoomSettings, roomToLeave?: SFSRoom);
}
...
/**
 * <b>Developers never istantiate the <em>PasswordUtil</em> class</b>: only use its static properties.
 */
export class PasswordUtil {
     ...
     *
     * @example
     * <caption>This example performs a login with an hashed password:</caption>
     * function doLogin()
     * {
     ...
     */
    static md5Password(pass: string): string;
}
